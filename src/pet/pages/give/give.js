...
Page({
    ...
    wx.chooseImage({
      ...
      success: function (res) {
        ...
        for (var i = 0; i < that.data.imgbox.length; i++) {
          ...
          wx.uploadFile({
            url: 'http://localhost:8080//SingleFile/upload', //里面填写你的上传图片服务器API接口的路径 
            filePath: that.data.imgbox[i],//要上传文件资源的路径 String类型 
            name: 'myfile',//按个人情况修改，文件对应的 key,开发者在服务器端通过这个 key 可以获取到文件二进制内容，(后台接口规定的关于图片的请求参数)
            header: {
  ...
  gbtn: function () {
    ...
    wx.request({
      url: 'http://localhost:8080/add_adopt',
      data: that.data,
      header: {
